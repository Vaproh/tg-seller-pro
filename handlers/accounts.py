...
from core.keyboards import category_keyboard, confirm_keyboard, pagination_keyboard, filter_keyboard
...
async def delete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Delete account #{account_id} ({esc(account['username'])})?",
...
async def bulkdelete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Enter account IDs (comma-separated) or a category name to delete all in that category:"
    )
