...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == "username":
        if len(text) > config.MAX_USERNAME_LEN:
            await update.message.reply_text(f"Username too long (max {config.MAX_USERNAME_LEN} chars).")
            return
    ...
    if stage == "password":
        if len(text) > config.MAX_PASSWORD_LEN:
            await update.message.reply_text(f"Password too long (max {config.MAX_PASSWORD_LEN} chars).")
            return
    ...
    if stage == "email":
        ...
        if len(text) > config.MAX_EMAIL_LEN:
            await update.message.reply_text(f"Email too long (max {config.MAX_EMAIL_LEN} chars).")
            return
    ...
    if stage == "notes":
        ...
        if len(text) > config.MAX_NOTES_LEN:
            await update.message.reply_text(f"Notes too long (max {config.MAX_NOTES_LEN} chars).")
            return
    ...
    if bulk_stage == "lines":
        ...
        if len(current) > 100000:
            await update.message.reply_text("Bulk input too large. Send /done to process what you have.")
            return
    ...
    if sell_stage == "buyer":
        if len(text) > config.MAX_BUYER_LEN:
            await update.message.reply_text(f"Buyer name too long (max {config.MAX_BUYER_LEN} chars).")
            return
    ...
    if sell_stage == "price":
        ...
        await update.message.reply_text("Add tags? (or /skip)")
        return
...
async def handle_csv_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if doc.file_size and doc.file_size > MAX_CSV_SIZE:
        await update.message.reply_text(f"CSV file too large (max {MAX_CSV_SIZE // (1024*1024)}MB).")
        return
