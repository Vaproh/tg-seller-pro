...
def revenue_chart(rows):
    ...
    ax1.set_ylabel("Revenue (₹)", color=color_bar, fontsize=11)
...
def top_buyers_bar(rows):
    if not rows:
        return None
    names = [str(r["buyer_name"])[:15] for r in reversed(rows)]
    ...
    bars = ax.barh(names, spent, color="#ffd93d", alpha=0.85, height=0.6)
    ax.set_xlabel("Total Spent (₹)", color="white", fontsize=11)

    for bar, count in zip(bars, sales):
        ax.text(bar.get_width() + max(spent) * 0.01, bar.get_y() + bar.get_height() / 2,
                f"₹{bar.get_width():.0f} ({count})", va="center", color="white", fontsize=9)

    _style_ax(ax, "🏆 Top Buyers")
...
def payment_donut(rows):
    ...
    total = sum(sizes)
    ax.text(0, 0, f"₹{total:.0f}", ha="center", va="center", fontsize=16, fontweight="bold", color="white")
...
def top_sellers_bar(rows):
    ...
    ax.set_xlabel("Total Revenue (₹)", color="white", fontsize=11)
